...
function OperationsModule({ name,role,initialFilter={} }: { name: string;role:string;initialFilter?:Record<string,string> }) {
  ...
  const paymentFields:Field[]=name==="Payments"?[
    ...
    {key:"project_id",label:"Project",dependsOn:"customer_selector",options:paymentLookups.projects.map(p=>({value:String(p.id),label:`${String(p.title||p.id)}${p.site_name?` · ${String(p.site_name)}`:""}`,parent:String(p.customer_id)}))},
