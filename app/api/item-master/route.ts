...
export async function GET(req: Request) {
  try {
    ...
    if (id) {
      ...
      const media = (await env.DB.prepare(`SELECT id,kind,file_key,source,review_status,review_issue,customer_approved,sort_order FROM product_media WHERE base_product_id=? ORDER BY sort_order`).bind(id).all()).results;
